...
function CategoryVisualization({ expenses }: { expenses: Expense[] }) {
...
function ExpenseDashboard2() {
  const [state, dispatch] = useReducer(reducer, initialState);
  const { search, category, page, pageSize } = state;
  ...
  const handleCategoryChange =
    useCallback((event: React.ChangeEvent<HTMLSelectElement>) => {
      dispatch({
        type: "SET_CATEGORY",
        payload: event.target.value
      });
...
export default ExpenseDashboard2;